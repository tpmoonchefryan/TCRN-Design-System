...
**施工**:
1. 先读 componentCss 现有刻度:`grep -oE -- '--tcrn-type-size-[a-z-]+: [^;]+' packages/ui-react/src/components/Navigation/Navigation.tsx`。把现有档位列表贴进 PR 描述。
2. 制定映射表(示例,按实际 token 值定):11px→`--tcrn-type-size-caption`、12px→`--tcrn-type-size-meta` 或 `-ui`、14-15px→`-ui`/`-body`、16-18px→`-section` 等;**10px 那 1 处**(alpha-styles,grep 定位)升 11px 或删除,10px 低于可读下限。
3. 若现有档位不够(如需要 13px 档),**先在 componentCss 增 token 再引用**,不许新字面量。
...
**施工**:
1. 读 space token 档位(componentCss `:root` 区),制定映射:2/4/6/8/12/16/18px 就近归档;**奇数档 7/5/3px 逐一裁决**(升或降到相邻档,PR 描述里列出每一处的选择及理由——这是设计判断,不是机械替换)。
2. `999px` 全部→`var(--tcrn-radius-pill)`。
3. componentCss 裸 `6px` 那处:并入最近的 radius token 或明确新 token;`--tcrn-state-chip-radius` 评估并入通用刻度(若值与 `-control` 相同则删私有 token)。
...
**施工**:对下列控件逐一补齐缺失态,并在 PR 描述里贴**矩阵表**(控件×三态,标注每格来源规则):
nav 组行 / 类目行 / 故事链接 / 故事披露头 / TableToolbar 搜索、chips、收起钮 / 侧栏收起钮 / 主题切换 / 语言菜单 / 章节 pager / on-this-page 链接 / 搜索结果项。
