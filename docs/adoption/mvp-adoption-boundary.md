# MVP Adoption Boundary

The first adoption path is token-first and copy-state-first.

## Allowed Now

- Synthetic examples using `@tcrn/ui-tokens`, `@tcrn/ui-copy-state`, and
  `@tcrn/ui-react`.
- Contract stories that demonstrate component props, keyboard labels, empty
  states, blocked states, and no-overclaim copy.
- Repo-local v1.0.0 Apache-2.0 release preparation for the Design System public
  package baseline.
- Current public GitHub repository and GitHub Release `v1.0.0` state may be
  referenced as public-surface state for this Design System repo only.

## Deferred

- AOS or TMS dependency changes.
- Product API, RBAC, backend DTO, route, persistence, evidence-truth, release,
  or deployment integration.
- npm package publication, hosted documentation deployment, Vercel deployment
  readiness, or private registry publication.
