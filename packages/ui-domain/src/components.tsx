...
// TCRN-DS-INIT-012 — the domain components, moved out of DS core.
//
// These name product entities: Work items, Knowledge pages, Gates, Evidence. They
// were built inside the Design System while it had one consumer, which made the
// library's own rule untrue (TCRN-TMS-MIN-008). Stage three of the move puts the
// implementations here; stages one and two established the package and repointed
// its consumer, so a regression now is attributable to this commit alone.
//
// Everything imported below comes from `@tcrn/ui-react`. The dependency runs
// domain → core and never the reverse; `pnpm generic:scan` in core is what keeps
// that true by refusing a domain component reaching core's component tree.
